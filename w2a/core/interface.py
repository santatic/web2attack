...
from w2a.lib.cmd import Cmd
from w2a.config import CONFIG
from w2a.lib.file import FullPath
from .core import Framework
from .options import Options
from .printer import color_status,color_success
...
class Interface(OverrideCmd):
	...
	( Có gì Hot  )
	 `--'`--'`--'
		...
		intro += fmt_string.format(CONFIG._NAME_, 'v' + CONFIG._VERSION_ + '') + os.linesep
	...
	def showOptions(self, options):
		...
		fmt_string = "  {0:<" + str(longest_name) + "} {1:<" + str(longest_value) + "} {2:<10} {3}"
		self.print_line(color_status(fmt_string.format('Name', 'Current Setting', 'Required', 'Description')))
		...
		for option_name in options.keys():
			...
			if option_val == None:
				option_val = ''
			option_id	= options.getOptionId(option_name)
			option_req	= 'no'
			if options.getOptionRequire(option_name):
				option_req	= 'yes'
			option_desc = options.getOptionHelp(option_name)
			list_options[option_id]	= fmt_string.format(option_name, str(option_val), str(option_req), option_desc)
	...
	def complete_info(self, text, line, begidx, endidx):
		return self.complete_use(text, line, begidx, endidx)#[i for i in self.frmwk.modules.keys() if i.startswith(text)]
	##########################################
	...
	def complete_set(self, text, line, begidx, endidx):
		try:
			if self.frmwk.current_module:
				options	= self.frmwk.modules[self.frmwk.current_module].options
				options	= dict(list(options.items()) + list(self.frmwk.modules[self.frmwk.current_module].advanced_options.items()))
			else:
				options = dict(list(self.frmwk.options.items()) + list(self.frmwk.advanced_options.items()))
			##################
			cmd = line.split(' ', 3)
			if len(cmd) < 3:
				...
				if completes != None:
					...
					if len(path) == 1:
						...
						dir	= path[0] + '/'
						com = path[1]
					dirs	= os.listdir(FullPath(dir))
					completes	= []
					for c in dirs:
						if os.path.isdir(FullPath(dir + c)):
							c = c + '/'
						completes.append(c)
	...
	def do_unset(self, name):
		...
		if name in options:
			options.unSetOption(name)
		elif name in advanced_options:
			advanced_options.unSetOption(name)
		else:
	...
	def do_show(self, args):
		...
		if args[0] == 'modules':
			self.print_line(os.linesep + color_success('Modules') + os.linesep + '=======' + os.linesep)
			longest_name = 20
			...
			self.print_line(color_status(fmt_string.format('Name', 'Description')))
			self.print_line(fmt_string.format('----', '-----------'))
			...
			if self.frmwk.current_module:
				if args[0] in ['options', 'all']:
					self.print_line(os.linesep + color_success('Module Options') + os.linesep + '==============' + os.linesep)
					self.showOptions(self.frmwk.modules[self.frmwk.current_module].options)
				if args[0] in ['advanced', 'all']:
					self.print_line(os.linesep + color_success('Advanced Module Options') + os.linesep + '=======================' + os.linesep)
					self.showOptions(self.frmwk.modules[self.frmwk.current_module].advanced_options)
			elif self.frmwk.current_module == None:
				if args[0] in ['options', 'all']:
					self.print_line(os.linesep + color_success('Framework Options') + os.linesep + '=================' + os.linesep)
					self.showOptions(self.frmwk.options)
				if args[0] in ['advanced', 'all']:
					self.print_line(os.linesep + color_success('Advanced Framework Options') + os.linesep + '==========================' + os.linesep)
					self.showOptions(self.frmwk.advanced_options)
			self.print_line()
		else:
			self.print_status('Valid parameters for the "show" command are: modules, options, advanced')
